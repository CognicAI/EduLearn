(function () {
    ...
    async function sendBotRequest(messageText, chatMessagesElement) {
        ...
        try {
            ...
            try {
                ...
                throw new Error("Cannot reach EduLearn server. Please ensure the backend is running and CORS is enabled.");
            }
            ...
            // Handle authentication and network errors specifically
            const msg = error.message.includes('Cannot reach') 
                ? 'Error: Cannot reach EduLearn server. Please check the backend.'
                : error.message.includes('Authentication failed') 
                    ? 'Your session has expired. Please refresh the page and log in again.'
                    : `Error: ${error.message}`;
            addMessageToChatUI(msg, 'error', chatMessagesElement);
        } finally {
    ...
    function createAndInjectUI() {
        ...
        chrome.storage.local.get(['backendType', 'webhookUrl'], (data) => {
            ...
            if (!sessionStorage.getItem('edulernChatbotVisited')) {
                sessionStorage.setItem('edulernChatbotVisited', 'true');
